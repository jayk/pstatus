...
test("a command-line config file takes precedence over PSTATUS_CONFIG", async () => {
  ...
  await writeFile(commandConfig, JSON.stringify({ files: { Command: ["command.md"] }, output: "command-output" }));
  ...
  assert.deepEqual(config.files, [{ label: "Command", paths: [path.join(root, "command.md")] }]);
});
